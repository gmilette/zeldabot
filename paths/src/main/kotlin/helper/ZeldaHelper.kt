...
class ZeldaHelper(private val monitor: ZeldaMonitor) {
    ...
    val plan = PlanRunner(
        { ZeldaPlan.makeMasterPlan(hyrule, hyrule.mapCellsObject, hyrule.levelMap) },
        api)
...
//        api.drawRect(strX - 2, strY - 2, strWidth + 3, 10)
    ...
    companion object {
        ...
        fun startIt(monitor: ZeldaMonitor) {
            ...
            ZeldaHelper(monitor).launch()
        }

        @JvmStatic
        fun main(args: Array<String>) {
            ApiSource.initRemoteAPI("localhost", 9999)
            ZeldaHelper(NoOp()).launch()
        }
    }
